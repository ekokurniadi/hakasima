...
class Penerimaan extends MY_Controller {
    ...
    {
        $this->Penerimaan_model->insert_temp();
    }

    public function checkout()
    {
    
       $this->Penerimaan_model->checkout2();
    }
    ...
    public function load_temp()
    {
        echo " <table class='table table-bordered'>
                    ...
                    $no=1;
                    $data = $this->Penerimaan_model->tabel_transaksi($user_id,$id)->result();
                    $period_array = array();
                    $total;
                    foreach ($data as $d) {
                        $total =$d->price * $d->qty;
    ...
    {
        $sku = $_POST['sku'];
        $data = $this->db->query("SELECT * FROM sku WHERE sku='$sku'")->result();
        foreach($data as $dd)
        {
            $data =array(
    ...
    public function index()
    {
        ...
        $tr = $this->Penerimaan_model->get_by_session()->result();

        $data = array(
            'dt' => $tr
        );
        $this->load->view('header');
        $this->load->view('penerimaan_list', $data);
    ...
    {
        $id=$_GET['id'];
        $this->Penerimaan_model->hapus_temp($id);
    }
    
    public function detail_transaksi($kode_transaksi)
    {
        $pengguna=$this->session->userdata('username');
        $tr = $this->Penerimaan_model->detail_transaksi($kode_transaksi)->result();

        $data = array(
            'dt' => $tr
        );
        $this->load->view('header');
        $this->load->view('tabel_detail_penerimaan', $data);
        $this->load->view('footer');
    }
    ...
    public function cetak_nota($kode_transaksi)
    {
       ...
       $data['nota']=$this->Penerimaan_model->kode_transaksi($kode_transaksi)->result();
       ...
       $dompdf->stream('Nota Penerimaan.pdf',array("Attachment"=>FALSE));
    }
    ...
    public function create() 
    {
        ...
        $data['kode'] = $this->Penerimaan_model->kode();
        $this->load->view('header');
        $this->load->view('penerimaan_form', $data);
         $this->load->view('footer');
    }
...
/* End of file Transaksi_pesanan.php */
/* Location: ./application/controllers/Transaksi_pesanan.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2019-04-07 22:12:35 */
/* http://harviacode.com */